...
const int windowSize = 1000;  // Window size
...
// Perform random walk with bias toward the center and along branches
void biasedRandomWalk(sf::Vector2i &particle) {
    ...
    // Random walk with bias towards the center
    int randomMove = rand() % 100;
    if (randomMove < 60) {
        particle.x += static_cast<int>(dx * 1.5);  // Move towards the center with more probability
        particle.y += static_cast<int>(dy * 1.5);
    } else {
...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize, windowSize), "Stella Dendrite Snowflake DLA");
